...
par (mfrow=c(1,2))
probs <- c(1:99)/100
qqqq <- quantile (x=(QQQ$rt - QQQ$tfit$Location)/QQQ$tfit$Scale, probs=probs)
qtheoretical <- qt (p=probs, df=QQQ$tfit$nu)

plot (qtheoretical, qqqq, pch=20, col='aquamarine4',
main = sprintf ("quantiles of QQQ log returns vs t(df=%.3f)", QQQ$tfit$nu),
xlab = sprintf ("Exact quantiles of t(df=%.3f)", QQQ$tfit$nu),
ylab = "Empirical quantiles of QQQ (rt - Location)/Scale"); grid()
abline (a=0, b=1, lty=2, col='red')


qqqq <- quantile (x=(QQQ$rt - mean (QQQ$rt))/sd(QQQ$rt), probs=probs)
qtheoretical <- qnorm (p=probs)
plot (qtheoretical, qqqq, pch=20, col='aquamarine4',
main = sprintf ("quantiles of QQQ log returns vs N(0,1)"),
xlab = sprintf ("Exact quantiles of N(0,1)"),
ylab = "Empirical quantiles of QQQ (rt - mu)/sigma"); grid()
abline (a=0, b=1, lty=2, col='red')
```
...
xlab="time", ylab="log monthly returns", main = "QQQ");grid()

acf (QQQ$rt^2, xlab="lags in months", ylab="correlation of rt^2",
main="Autocorrelation of (log (QQQ monthly returns))^2"); grid()
```
...
{
x <- seq (from = -3.5, to=3.5, length=100)
yn <- dnorm (x)

df <- 4 * (exp(log(50/4)/7)^c(0:7))
yt <- matrix (rep(0.0, length(x)*length(df)), nrow=length(df))

for (i in 1:length(df)) {
  s <- sqrt ((df[i]-1)/df[i])
  yt[i,] <- dt (x/s, df=df[i]) / s
}


plot (x, yn, type='l', col='black', xlim = c(-3.5,3.5),
      ylim = c(0.0, 0.5),
      xlab="x", ylab="density",
      main="Comparing variance standardized t densities vs N(0,1)"); grid()

mycols <- rainbow (length(df))
for (i in 1:length(df)) {
  lines (x, yt[i,], lty=2, col=mycols[i])
}

tmp <- sapply (1:length(df), function(i) {paste("df=", round(df[i],1), ", var=1",sep='')})

legend (x=-3.4, y=0.48, 
        legend = c ("normal, var=1", tmp),
        col=c("black", mycols), pch= rep(NULL, length(df)+1),
        lty=rep(2,length(df)+1))
}
```
...
par (mfrow=c(2,2))
acf (XEG.TO$rt^2, xlab="lags in months", ylab="correlation of rt^2",
main="Autocorrelation of XEG.TO (log (monthly returns))^2"); grid()
plot (XEG.TO$the_months[2:XEG.TO$m], XEG.TO$rt, type='h', col='mediumslateblue',
main="XEG.TO log monthly returns",
xlab = "time", ylab=expression(r[t])); grid();
lines (XEG.TO$the_months[2:XEG.TO$m], XEG.TO$trend$ct, col='red')
lines (XEG.TO$the_months[2:XEG.TO$m], XEG.TO$trend$st, col='aquamarine4')
legend (x=2002, y=-0.1, legend=c("location", "scale"), lty=c(1,1),
col=c('red', 'aquamarine4'), pch=c(NA,NA))

acf (XEG.TO$trend$zt^2, xlab="lags in months", ylab="correlation of (residuals)^2",
main="Autocorrelation of XEG.TO (zeta_t)^2 process"); grid()
```
